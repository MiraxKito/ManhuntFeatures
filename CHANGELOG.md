...
## 1.0.2 — Manhunt game mode and compass improvements

Added:

- Optional `/mhfeatures start` and `/mhfeatures stop` game commands.
- Internal game timer that does not replace other scoreboard plugins.
- Automatic hunter and runner win detection.
- Runner elimination handling with spectator or original-world fallback.
- Hunter spawn placement in the Manhunt Overworld at game start.
- Tracked-player chat messages when using the Manhunt Compass.
- A visible enchantment glow on the special Manhunt Compass.
- English and Russian localization files.
- In-game admin configuration panel via `/mhfeatures config`.
