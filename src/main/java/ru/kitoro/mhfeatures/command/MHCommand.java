...
public final class MHCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "config": config(sender); break;
    ...
    private void role(CommandSender sender, String[] args) {
        ...
        if (plugin.getGameManager().isActive()) { sender.sendMessage(plugin.message("roles-locked-game")); return; }
        if (args.length < 3) { sender.sendMessage(plugin.message("usage-role")); return; }
        Role role = Role.parse(args[2]);
        if (role == null) { sender.sendMessage(plugin.message("role-invalid")); return; }
        List<Player> targets = select(sender, args[1]);
        if (targets.isEmpty()) { sender.sendMessage(plugin.message("no-players")); return; }
        for (Player target : targets) {
            ...
            target.sendMessage(plugin.message("your-role", "%role%", role.name().toLowerCase(Locale.ROOT)));
        }
        sender.sendMessage(plugin.message("role-applied", "%count%", targets.size()));
    }
    ...
    private void reset(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            catch (NumberFormatException exception) { sender.sendMessage(plugin.message("seed-invalid")); return; }
        }
    ...
    private void compass(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(plugin.message("compass-given", "%count%", targets.size()));
    }
    ...
    private void teleport(CommandSender sender, String[] args, boolean toMh) {
        ...
        if (world == null) { sender.sendMessage(plugin.message("target-world-not-found")); return; }
        for (Player target : targets) target.teleport(world.getSpawnLocation());
        sender.sendMessage(plugin.message("players-moved", "%count%", targets.size()));
    }

    private void start(CommandSender sender) {
        if (!sender.hasPermission("mhfeatures.admin.game")) { deny(sender); return; }
        if (plugin.getGameManager().isActive()) {
            sender.sendMessage(plugin.message("already-running"));
            return;
        }
        if (plugin.getRoleManager().count(Role.HUNTER) < 1 || plugin.getRoleManager().count(Role.RUNNER) < 1) {
            sender.sendMessage(plugin.message("start-requires-roles"));
            return;
        }
        if (plugin.getWorldManager().getOverworld() == null
                || Bukkit.getWorld(plugin.getWorldManager().netherName()) == null
                || Bukkit.getWorld(plugin.getWorldManager().endName()) == null) {
            sender.sendMessage(plugin.message("start-requires-worlds"));
            return;
        }
    ...
    private void stop(CommandSender sender) {
        if (!sender.hasPermission("mhfeatures.admin.game")) { deny(sender); return; }
        if (!plugin.getGameManager().stop(null)) sender.sendMessage(plugin.message("no-game-running"));
    }
    ...
    private void status(CommandSender sender) {
        ...
        if (plugin.getGameManager().isActive()) {
            ...
            sender.sendMessage(plugin.message("status-game-idle"));
        }
        String worlds = (Bukkit.getWorld(plugin.getWorldManager().overworldName()) != null ? "Overworld " : "")
                + (Bukkit.getWorld(plugin.getWorldManager().netherName()) != null ? "Nether " : "")
                + (Bukkit.getWorld(plugin.getWorldManager().endName()) != null ? "End" : "");
        sender.sendMessage(plugin.message("status-worlds", "%worlds%", worlds));
        String players = plugin.messageRaw("status-players")
                .replace("%hunters%", String.valueOf(plugin.getRoleManager().count(Role.HUNTER)))
                .replace("%runners%", String.valueOf(plugin.getRoleManager().count(Role.RUNNER)));
        sender.sendMessage(plugin.messageValue(players));
    }

    private void config(CommandSender sender) {
        if (!sender.hasPermission("mhfeatures.admin.config")) { deny(sender); return; }
        ...
        plugin.getConfigMenu().open((Player) sender);
    }
    ...
    private void reload(CommandSender sender) {
        ...
        plugin.reloadConfig();
        plugin.loadMessages();
        sender.sendMessage(plugin.message("configuration-reloaded"));
    }
    ...
    private void deny(CommandSender sender) { sender.sendMessage(plugin.message("permission-denied")); }
    private void help(CommandSender sender) { sender.sendMessage(plugin.message("help")); }

    @Override
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) return partial(args[0], Arrays.asList("role", "reset", "compass", "mhworld", "ogworld", "start", "stop", "status", "config", "reload"));
